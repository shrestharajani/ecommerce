...
export const CartDrawer = ({ onClose, visible }) => {
    ...
    <>
      ...
      <div className="checkout-buttons">
        <Button
          ...
          ghost
        >
          <Link to="/add-to-cart" onClick={onClose}>
            View Cart
          </Link>
        </Button>
  ...
  const deleteCartItem = (itemId) => {
    const index = storageCartItem.cart_item.findIndex(
      ({ id }) => id === itemId
    );
    if (index === -1) {
      alert("No id found");
      ...
      const updatedTotal =
        storageCartItem.total - item[0].price * item[0].quantity;
      const updatedCount = storageCartItem.cartItemCount - 1;
      dispatch(
        itemDeletedFromCart(
          storageCartItem.cart_item,
          updatedTotal,
          updatedCount
        )
      );
  ...
  return (
    <>
      ...
      >
        <h3>Your Cart</h3>
        {storageCartItem.cart_item.length === 0 && (
